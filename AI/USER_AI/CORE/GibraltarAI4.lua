...
---------------------------------------------------------------------------
	...
	if (not Started) then
		...
		TimerToUseSkill = GetTick() + 2000
		
		TimerToReact    = GetTick() + 1000
...
---------------------------------------------------------------------------
	...
	if (not AnyoneCouldMove) then
		for i, v in pairs(Motions) do 
			--if (Friends[i] ~= nil or i == MyEnemy or i == MyNearestEnemy or i == MySecondaryEnemy) then
...
---------------------------------------------------------------------------
	...
	if GetTick() >= StateTimeOut[MyState] and MyState == FOLLOW_ST then
		...
		if MyEnemy > 0 and GetDistance2(MyID, OwnerID) <= 7 then 
			...
			if GetDistance2(MyID, OwnerID) <= 7 then MyDestX, MyDestY = RepairDest(MyID, MyX, MyY); Move(MyID, MyDestX, MyDestY); end
...
---------------------------------------------------------------------------
	...
	if (MyEnemy == 0 or IsKS(MyEnemy) or IsOutOfSight(GetV(V_TARGET, MyEnemy), MyEnemy)) then
		...
		StateTimeOut[MyState] = nil
		TimerToChangeTarget = 0
		MyDestX, MyDestY = RepairDest(MyID, MyX, MyY); Move(MyID, MyDestX, MyDestY); 
	...
	-- local attack = false
	-- if (MyNearestEnemy > 0) then
		-- if (IsInAttackSight(MyID, MyNearestEnemy)) then
			-- attack = true
		-- end
	-- end
	-- if (MyEnemy > 0) then
	...
	--end
	if (MyMotion == MOTION_MOVE) and Motions[MyEnemy] == MOTION_MOVE then
		RemoveCall(ATTACK);
	end 
	if (MyMotion == MOTION_MOVE) then
			...
			if (List.containsV2(CCommands, SKILL_OBJECT)) then
				...
				and skill ~= 8013) then
					...
					and BygoneOwnerMotion ~= MOTION_ATTACK2
						...
						local eX, eY = GetV(V_POSITION, id);
						local x, y; 
						local radius = range
...
---------------------------------------------------------------------------
	...
	-- if (MyEnemy > 0) then
		...
		-- end
	-- end
	--if (MyMotion ~= MOTION_MOVE or attack) then
		if (MyNearestEnemy > 0) then
			--if (GetDistance2(MyID, MyNearestEnemy) <= MyAttackRange) then
				Attack(MyID, MyNearestEnemy)
			--end
